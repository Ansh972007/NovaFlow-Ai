...
def safe_upload_id(upload_id: str) -> str:
    """Reject path traversal / non-hex upload ids."""
    uid = (upload_id or "").strip()
    if not re.fullmatch(r"[a-fA-F0-9\-]{8,64}", uid):
        raise FileSecurityError("Invalid upload id")
    if ".." in uid or "/" in uid or "\\" in uid:
        raise FileSecurityError("Invalid upload id")
    return uid


def safe_subdir(base: Path, *parts: str) -> Path:
    """Resolve a child path and ensure it stays under base."""
    root = base.resolve()
    path = root.joinpath(*parts).resolve()
    try:
        path.relative_to(root)
    except ValueError as exc:
        raise FileSecurityError("Invalid upload path") from exc
    return path
...
def validate_upload_metadata(
    *,
    ...
    When ``head`` is empty (chunk-init before first bytes arrive), magic checks are skipped.
    Pass ``max_bytes`` to enforce a custom ceiling (e.g. chat 2 GiB); otherwise uses MAX_UPLOAD_BYTES
    unless ``bypass_max_size`` is True.
    """
    if size <= 0:
        raise FileSecurityError("Empty upload")
    limit = max_bytes if max_bytes is not None else MAX_UPLOAD_BYTES
    if not bypass_max_size and size > limit:
        raise FileSecurityError(f"File exceeds maximum size of {limit} bytes")
    ...
    head = head or b""
    # Skip magic when head not available yet (chunk-init)
    if len(head) >= 2:
        if head.startswith(b"MZ") or head.startswith(b"\x7fELF"):
            raise FileSecurityError("Executable files are not allowed")
        if head.lstrip().startswith(b"#!"):
